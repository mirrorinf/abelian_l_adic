...
志村五郎在\parencite{shimura1966reciprocity}中讨论了由
定义在$\mathbb{Q}$上的椭圆曲线$E$的$\ell$阶点生成的数域$L/\mathbb{Q}$，
其中$\ell$是一个素数.
...
\end{equation}
的Fourier系数中. 那么，$L$中素数的分解性质就（几乎，还差确定幺幂共轭类）被编码在了一个模形式中.
...
\begin{enumerate}
    ...
    \item 在Frobenius元素的特征多项式有重根时，如何确定它是不是数乘？
...
可以将所有的$\rho_n$放在一起考虑. 记$E[\ell^{\infty}]=\bigcup_n E[\ell^n]$，$K_{\infty} = K(E[\ell^{\infty}])$. $K_{\infty}$也是Galois扩张，且$\mathrm{Gal}(K_{\infty}/K) = \lim\limits_{\longleftarrow} \mathrm{Gal}(K_n/K)$. 记$\rho = \lim\limits_{\longleftarrow} \rho_n$，$\rho: G\to \mathrm{GL}_2(\mathbb{Z}_{\ell})$，其中转移映射$\mathrm{GL}_2(\mathbb{Z}/\ell^{n+1} \mathbb{Z}) \to \mathrm{GL}_2(\mathbb{Z}/\ell^n \mathbb{Z})$，$\tau \mapsto \tau \pmod{\ell^n}$.
