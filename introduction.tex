...
\begin{cthm}
    $K, E$同上. 对于除了有限多个以外的素数$\ell$，$\pmod{\ell}$Galois表示$\overline{\rho}_{\ell}: \mathrm{Gal}(\overline{K}/K)\to \mathrm{GL_2}(\mathbb{F}_{\ell})$是满射.
\end{cthm}

这两个定理的证明都分为部分：利用不同素数$\ell$的$\ell$进Galois表示之间的相容性限制$\rho_{\ell}$的行为；对$\mathrm{GL}_2$（这里的模糊时故意的，两种情况下的$\mathrm{GL}_2$是不一样的）子群分类.
