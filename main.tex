\documentclass[a4paper, UTF8, CJKmath=true]{ctexart}

\usepackage{mathtools}
\usepackage{graphicx}
\usepackage{amsthm}
\usepackage{mathrsfs}
\usepackage{amsfonts}
\usepackage{hyperref}
\usepackage[mathrm=sym,math-style=french]{unicode-math}
\usepackage{tikz-cd}
\usepackage{float}

\usepackage{xcolor}
\definecolor{yunshanlan}{RGB}{47, 144, 185}
\definecolor{special_argument}{RGB}{10, 100, 100}

\usepackage[left=2.5cm,right=2.5cm,top=3cm,bottom=3cm]{geometry}

\setmainfont[
    BoldFont={HYXuanSong-75S},
]{HYXuanSong}
\setCJKmainfont[
    BoldFont={HYXuanSong-75S},
]{HYXuanSong}

\usepackage[backend=biber]{biblatex}
\addbibresource[location=local]{main.bib}

\theoremstyle{definition}

\newtheorem{cthm}{定理}
\newtheorem{cprop}{命题}
\newtheorem{crem}{注记}
\newtheorem{cdef}{定义}
\newtheorem{ccor}{推论}
\newtheorem{clem}{引理}
\newtheorem{cyyy}{白日梦}

\newenvironment{pppmatrix}{\left(\begin{matrix}}{\end{matrix}\right)}

\newcommand{\paten}[1]{\left(#1\right)}
\newcommand{\deri}{\mathrm{d}}
\newcommand{\I}{\mathrm{i}}
\newcommand{\modular}{\mathcal{M}}
\newcommand{\cusp}{\mathcal{S}}
\newcommand{\diam}[1]{\big<#1\big>}
\newcommand{\matbt}[4]{\begin{pppmatrix}#1 &#2\\#3 &#4\end{pppmatrix}}
\newcommand{\absn}[1]{\left|#1\right|}
\newcommand{\order}{\mathcal{O}}

\newcommand{\invlim}{\lim\limits_{\longleftarrow}}

\setlength{\parindent}{0em}

\title{椭圆曲线定义的Galois表示的像}
\author{褚晓敏}
\date{\today}

\begin{document}
    \maketitle

    \input{introduction.tex}

    \input{representation.tex}

    \input{open_image.tex}

    \input{full_image.tex}

    \printbibliography[heading=bibliography,title=参考文献]
\end{document}