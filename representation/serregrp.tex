...
现在来证明定理\ref{single_ell}的(3). 回顾，如果$H$是$k$上的代数群，记$\mathsf{Rep}_k(H)$为$H$的有限维半单表示的范畴. 当$k_1$是$k$的扩张是，有自然的$-\otimes k_1: \mathsf{Rep}_k(H)\to \mathsf{Rep}_{k_1}(H/k_1)$. 如果$\rho\in \mathsf{Rep}_{k_1}(H/k_1)$落在$-\otimes k_1$的像（essential image）中，则称$\rho$可以定义在$k$上.
...
% \subsection{\texorpdfstring{$T_{\mathfrak{m}}$}{Tm}的结构}

% 本小节中$G = \mathrm{Gal}(\overline{\mathbb{Q}}/\mathbb{Q})$.

% 设$\omega$是$\overline{\mathbb{Q}}$的一个复嵌入，记复共轭诱导的$\overline{\mathbb{Q}}$的自同构为$c_{\omega}$. 那么对不同的$\omega$，$c_{\omega}$形成了一个共轭类，记为$C_{\infty}$.

% 将$X(T)$写成加法形式，令$Y(T) = X(T)\otimes_{\mathbb{Z}} \mathbb{Q}$. 令
