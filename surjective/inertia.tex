...
比如说，由$\overline{\rho}_{\ell}$交换不能直接推出$\rho_{\ell}$交换.
所以还需要多挖掘一些$\overline{\rho}_{\ell}$的信息.
...
现在回到对$\overline{\rho}_{\ell}$的讨论.
注意到Weil配对给出了保持$G$作用的同构$\wedge^2 E[\ell] \to \mu_{\ell}$.
那么$\overline{\rho}_{\ell}$的行列式$G\to \mathbb{F}_{\ell}^{\times}$
就是$G$在$\mu_{\ell}$上的作用.
